...
class IndependencePipeline:

    # Database setup
    conn = sqlite3.connect('independence.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `independence`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM independence WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO independence (date, title, link, content)"
